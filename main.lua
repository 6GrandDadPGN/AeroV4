...
local function downloadPremadeProfiles()
    ...
    local success, response = pcall(function()
        return game:HttpGet('https://api.github.com/repos/6GrandDadPGN/AeroV4/contents/profiles/premade?ref=' .. commit)
    end)
            ...
            for _, file in pairs(files) do
                if file.name and file.name:find('.txt') and file.name ~= 'commit.txt' then
                    ...
                    if not isfile(filePath) then
                        local dl = file.download_url or ('https://raw.githubusercontent.com/6GrandDadPGN/AeroV4/' .. commit .. '/profiles/premade/' .. file.name)
        ...
        if (not teleportedServers) and (not shared.VapeIndependent) then
            ...
            local teleportScript = [[
                ...
                if shared.VapeDeveloper then
                    ...
                    loadstring(game:HttpGet('https://raw.githubusercontent.com/6GrandDadPGN/AeroV4/'..readfile('newvape/profiles/commit.txt')..'/loader.lua', true), 'loader')()
                end
            ]]
            if shared.VapeDeveloper then
...
if not shared.VapeIndependent then
    ...
    if isfile('newvape/games/' .. gameFileName) then
        ...
        if not shared.VapeDeveloper then
            local suc, res = pcall(function()
                return game:HttpGet('https://raw.githubusercontent.com/6GrandDadPGN/AeroV4/' .. readfile('newvape/profiles/commit.txt') .. '/games/' .. gameFileName, true)
            end)
